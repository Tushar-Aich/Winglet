...
const trendingTweets = AsyncHandler(async (req: Request, res: Response) => {
  const top10Tweets = await TweetModel.aggregate([
    {
      $match: {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, top10Tweets, "Tweets Fetched Successfully"));
});
